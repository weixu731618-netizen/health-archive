...
// ⚠️ 上架前这份阈值表需要临床人员过一遍。命中只出「建议尽快就医复核」，不诊断。
final List<_Rule> _rules = [
  ...
  _Rule(['中性粒细胞'], (v) => v >= 0 && v < 0.5, '中性粒细胞绝对值极低（粒细胞缺乏）',
      not: ['百分比', '比率']),
  // 只认真正提示血液系统恶性的：原始 / 幼稚 / 母细胞。
  // 早幼粒 / 中幼粒 / 晚幼粒（粒细胞左移）在重症感染里也常见，不进这条。
  _Rule([
    '原始细胞', '幼稚细胞', '原幼', '幼稚粒', '母细胞', '原幼稚',
  ], (v) => v > 0, '外周血出现原始 / 幼稚细胞'),
  ...
  _Rule(['钙', 'calcium'], (v) => v >= 3.5 && v < 10, '血钙过高（高钙危象风险）',
      not: ['尿', '磷']),
  _Rule(['钙', 'calcium'], (v) => v > 0.5 && v < 1.75, '血钙过低（危急值）',
      not: ['尿', '磷']),
